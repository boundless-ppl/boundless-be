...
type emailLoginComponent struct {
	*login.Login
}

func newEmailLoginComponent() *emailLoginComponent {
	return &emailLoginComponent{Login: login.GetLoginComponent()}
}

func (c *emailLoginComponent) GetTemplate() (*htmltemplate.Template, string) {
	tmpl, err := htmltemplate.New("login_theme1").
		Funcs(login.DefaultFuncMap).
		Parse(emailLoginTmpl)
	if err != nil {
		panic(err)
	}
...
const emailLoginTmpl = `{{define "login_theme1"}}
<!DOCTYPE html>
<!--[if lt IE 7]>
